...
addr = '0x40ECBb60736Ea55E8E05383CcEc85f01ba871cf0'
...
def ensure_all_users_have_wallets():
	c = frappe.get_all('Institution Member', filters= dict(member_status="Active"), fields=['name'])
	for i in c:
		w = create_wallet()
		voter = i.get("name")
		doc = frappe.get_doc("Institution Member", i.get("name"))
		doc.db_set("public_key",w.get("public_key"))
		doc.db_set("private_key",w.get("private_key"))
		print(f"{voter}{w}")
		# frappe.db.set_value('Insteitution Member', i['name'], {
...
def send_ethers_to_all_users(nonce):
	...
	nonce = nonce
	for i in c:
		print(send_ethers(i['public_key'], 100000000000000000000, nonce=nonce))
		nonce = nonce + 1
		# time.sleep(1)
