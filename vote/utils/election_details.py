	...
	frappe.local.response.update(
		{
			...
			"status": "error",
			"error": str("Service is currently unavailable."),
		}
	)
	return
	# end here
	search_args = dict(name=voter_id, id_number=member_id, member_status="Active")
	...
	try:
		...
		def validate_ballot(election=None, ballot=None, voter=None):
			...
			for branch_position in advertised_positions:
				...
				context = list(
					filter(
						lambda x: x.get("branch") == branch_position.get("branch")
						and x.get("position") == branch_position.get("position"),
						ballot_data,
					)
				)
				if len(context) > int(maximum_posts):
		...
		# validity_status = validate_ballot(election=election, ballot=ballot_data, voter=voter)
		...
		for position in distinct_branch_positions:
			...
			def _get_branch_position_tally(position_context=position_context):
				...
				for j in position_context:
					...
					context_tally.append(row)
				sorted_tally = sorted(
					context_tally, key=lambda k: k["vote_count"], reverse=True
				)
				return sorted_tally
...
def handle_unposted_ballots():
	unposted = frappe.get_all(
		"Ballot Entry",
		filters=dict(posted_to_blockchain=False, no_wallet=True),
		fields=["name"],
	)
	if not unposted:
		return
	length = len(unposted)
	# print(f"{length} to be handled {unposted}")
	...
	voters = []
	for j in docs:
		if j.get("voter_id") in voters:
			continue
		j.process_blockchain()
		voters.append(j.get("voter_id"))
...
@frappe.whitelist(allow_guest=True)
def national_voter_breakdown(election=None):
	sql = """
	SELECT candidate_id, candidate_name, voter_branch, (SELECT parent_electoral_district FROM `tabElectoral District` WHERE name = a.voter_branch) AS region, sum(choice)

	FROM `tabBallot Entry Detail` a

	WHERE choice = 1 AND position LIKE '%nation%'

	AND parent in (SELECT name FROM `tabBallot Entry` WHERE election = '{}' and tallied = 1) 

	GROUP BY candidate_id,voter_branch  ORDER BY position,candidate_name  ASC;
	""".format(election)
	return frappe.db.sql(sql,as_dict=1)


# def voter_break_down(election=election):
#     def get_all_candidates_in_an_election(election=None):
#         pass

#     def get_branch_count(ballot_entry=None, candidate=None, branch=None):
#         pass

#     def get_branch_ballots(election=None, branch=None):
#         return frappe.get_all("Ballot Entry", filters=dict(branch=branch), fields=["*"])

#     candidates = get_all_candidates_in_an_election(election=election)
#     all_branches = frappe.get_all(
#         "Electoral District", fields=["name"], order_by="name asc"
#     )
...
#             branch_ballots = get_branch_ballots(election=election, branch=branch_name)
#             votes = len(list(filter(lambda x: x.get("candidate_id")==candidate_name and x.get("choice")==1, branch_ballots)))
